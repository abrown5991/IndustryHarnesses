#!/usr/bin/env bash
# One-command build + deploy of the Automotive Plant-Ops harness to AgentCore Runtime.
#
...
REPO="${REPO:-automotive-mfg-harness}"
RUNTIME_NAME="${RUNTIME_NAME:-automotive_mfg_harness}"
ROLE_NAME="${ROLE_NAME:-AutomotiveMfgHarnessRuntimeRole}"
...
aws iam put-role-policy --role-name "$ROLE_NAME" \
  --policy-name AutomotiveMfgHarnessRuntimePermissions \
...
echo ">> Upload KB docs: aws s3 cp <file> s3://automotive-mfg-harness-kb-docs/"
