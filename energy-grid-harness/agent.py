"""Energy Grid-Ops harness — AgentCore Runtime entrypoint.
...
The agent bundles: a grid-ops system prompt, a primary Claude model on Bedrock,
and a set of grid tools (currently mock stand-ins for real MCP servers).
"""
...
app = FastAPI(title="Energy Grid-Ops Harness", version="0.1.0")

_SYSTEM_PROMPT = (Path(__file__).parent / "prompts" / "grid_ops_system.md").read_text()
...
def _build_agent(model_id: str) -> Agent:
    return Agent(
        ...
        tools=[*GRID_TOOLS, retrieve_knowledge_base],
    )
