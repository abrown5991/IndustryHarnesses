"""Eval runner for the energy grid-ops harness.
...
def _print_report(reports: List[CaseReport], offline: bool, judge: bool) -> int:
    ...
    print(f"\n=== Energy Grid-Ops Harness Evals — layers: {layers} ===\n")
    n_pass = 0
    for r in reports:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run energy grid-ops harness evals.")
