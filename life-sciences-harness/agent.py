"""Life-Sciences Research harness — AgentCore Runtime entrypoint.
...
The agent bundles: a research system prompt, a primary Claude model on Bedrock,
and a set of research tools (currently mock stand-ins for real MCP servers).
"""
...
app = FastAPI(title="Life-Sciences Research Harness", version="0.1.0")

_SYSTEM_PROMPT = (Path(__file__).parent / "prompts" / "research_system.md").read_text()
...
def _build_agent(model_id: str) -> Agent:
    return Agent(
        ...
        tools=[*RESEARCH_TOOLS, retrieve_knowledge_base],
    )
