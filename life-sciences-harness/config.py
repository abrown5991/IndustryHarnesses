...
# Primary model: complex reasoning (hypothesis generation, cross-study synthesis,
# multi-omics interpretation, experimental design critique).
...
# Fast/cheap model tier: routine lookups, database queries, and summaries.
# TODO(verify): confirm exact Bedrock model/inference-profile ID for your region.
...
# Sampling / generation controls. Scientific work favors low temperature for
# accurate, well-grounded, citable answers.
...
# Safety-sensitive prompts (biosecurity, regulated records, physical controls,
# individual clinical decisions) force-route to PRIMARY regardless of complexity.
ROUTING_ESCALATE_SAFETY = (
    os.getenv("ROUTING_ESCALATE_SAFETY", "true").lower() == "true"
)

# --- Bedrock Knowledge Base (RAG) ---------------------------------------------
# KB_ID: the Bedrock Knowledge Base ID created by setup_kb.py.
# Leave empty to disable RAG (the tool returns a NOT_CONFIGURED note).
KB_ID = os.getenv("KB_ID", "")

# Number of passages to retrieve per query.
KB_RESULTS_COUNT = int(os.getenv("KB_RESULTS_COUNT", "5"))

# S3 bucket that backs this harness's knowledge base.
# Used by setup_kb.py; not needed at agent runtime.
KB_S3_BUCKET = os.getenv("KB_S3_BUCKET", "life-sciences-harness-kb-docs")
