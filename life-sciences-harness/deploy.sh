#!/usr/bin/env bash
# One-command build + deploy of the Life-Sciences Research harness to AgentCore Runtime.
#
...
REPO="${REPO:-life-sciences-harness}"
RUNTIME_NAME="${RUNTIME_NAME:-life_sciences_harness}"
ROLE_NAME="${ROLE_NAME:-LifeSciencesHarnessRuntimeRole}"
...
aws iam put-role-policy --role-name "$ROLE_NAME" \
  --policy-name LifeSciencesHarnessRuntimePermissions \
...
echo ">> Upload KB docs: aws s3 cp <file> s3://life-sciences-harness-kb-docs/"
